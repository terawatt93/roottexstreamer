...
void example_beamer()
{
	 ...
	 * */
	doc<<"title={Название первого слайда}";
	doc<<"subtitle={Подзаголовок}";
	doc<<"Это очень важный документ, который демонстрирует возможности TEXStreamer. Вот, например, нарисуем гистограмму из root:\n";
	...
	doc<<fig;
	doc<<string("\\newpage");//перейдем на следующий слайд
	doc<<"Действительно. Нужен еще мем сбоку\n";
	...
	doc<<tab;
	ofstream ofs("example_beamer.tex");
	ofs<<doc.Generate();
	ofs.close();
}
